...
const PLUGIN_NAME = _aprbResolvePluginName("APRealtimeBattle_AllInOne");
  const _APRB_VERSION = 'v2.1.10-fix7-keepFrameRefresh1-uiVisibilityLatch5-stallActNowKick1-forceBmAlwaysIfNotCalled-cinMsgInputFix1-iconHideLatch8-battleTimeFreezeFix4-actionDisableTag1';
  const _APRB_BUILD_DATE = "2026-02-13";
...
// --- Battle-time freeze guard (global) ---
// Freeze "battle time progress" (TBP, keepframe, delay timers, etc.) while messages / troop events / common events are active.
// IMPORTANT: Do NOT stop BattleManager.update itself, or the battle can deadlock. This guard only prevents APRB's time-progress side effects.
function APRB_isBattleTimeFrozen(bm) {
  try { if (!bm) bm = (typeof BattleManager !== "undefined") ? BattleManager : null; } catch (_) { bm = null; }
  try {
    const explicit = !!(bm && (bm._aprbTimeStop || bm._aprbParryExternalPause || bm._aprbCinematicAfterCommonActive));
    const msgBusy = !!($gameMessage && $gameMessage.isBusy && $gameMessage.isBusy());
    const evRun = !!($gameTroop && $gameTroop.isEventRunning && $gameTroop.isEventRunning());
    const ceRes = !!($gameTemp && $gameTemp.isCommonEventReserved && $gameTemp.isCommonEventReserved());
    return (explicit || msgBusy || evRun || ceRes);
  } catch (_) {
    return false;
  }
}
try {
  if (typeof BattleManager !== "undefined") {
    BattleManager.APRB_isBattleTimeFrozen = function() { return APRB_isBattleTimeFrozen(this); };
  }
} catch (_) {}
...
function APRB_processTbpFrameProgress(bm) {
  try {
    ...
    // freeze: do not progress any frame-based battle time while events/messages/common events are active
    if (APRB_isBattleTimeFrozen(bm)) return;
...
// ============================================================
// APRB Action Disable (State Notetag)
//   State note tag example:
//     <APRB_ActionDisable>
// - Battler with ANY such state is treated as canMove() === false.
// - Player input / Enemy AI are both blocked.
// ============================================================
function APRB_stateHasActionDisableTag(stateId) {
  try {
    const st = $dataStates ? $dataStates[stateId] : null;
    if (!st) return false;
    ...
    // 1) Meta (preferred)
    const meta = st.meta || {};
    // Common key variants (case-sensitive in MZ)
    if (meta.APRB_ActionDisable != null) return !!meta.APRB_ActionDisable;
    if (meta.APRBActionDisable != null) return !!meta.APRBActionDisable;
    if (meta.aprbActionDisable != null) return !!meta.aprbActionDisable;
    if (meta.AprbActionDisable != null) return !!meta.AprbActionDisable;
    if (meta.ActionDisable != null) return !!meta.ActionDisable;
    if (meta.actionDisable != null) return !!meta.actionDisable;

    // 2) Robust scan (case-insensitive)
    const keys = Object.keys(meta);
    for (let i = 0; i < keys.length; i++) {
      const k = String(keys[i] || '');
      const kk = k.toLowerCase().replace(/_/g, '');
      if (kk === 'aprbactiondisable' || kk === 'actiondisable') return !!meta[k];
    }

    // 3) Raw note fallback (rare)
    const note = String(st.note || '');
    if (/<\s*APRB_ActionDisable\s*>/i.test(note)) return true;
    if (/<\s*APRBActionDisable\s*>/i.test(note)) return true;
    if (/<\s*aprb_action_disable\s*>/i.test(note)) return true;
    if (/<\s*ActionDisable\s*>/i.test(note)) return true;
  } catch (e) {}
  return false;
}

if (typeof Game_BattlerBase !== "undefined") {
  if (!Game_BattlerBase.prototype.isAprbActionDisabled) {
    Game_BattlerBase.prototype.isAprbActionDisabled = function() {
      try {
        const ids = this._states || [];
        for (let i = 0; i < ids.length; i++) {
          if (APRB_stateHasActionDisableTag(ids[i])) return true;
        }
      } catch (e) {}
      return false;
    };
  ...
  // Apply to canMove() so existing APRB checks keep working.
  if (!Game_BattlerBase.prototype._aprbCanMoveWrap_ActionDisable) {
    Game_BattlerBase.prototype._aprbCanMoveWrap_ActionDisable = true;
    const _APRB_GBB_canMove = Game_BattlerBase.prototype.canMove;
    Game_BattlerBase.prototype.canMove = function() {
      const base = _APRB_GBB_canMove ? _APRB_GBB_canMove.call(this) : true;
      if (!base) return false;
      try {
        if (this.isAprbActionDisabled && this.isAprbActionDisabled()) return false;
      } catch (e) {}
      return base;
    };
  }
  ...
  Scene_Battle.prototype.update = function() {
    ...
    try {
      ...
      if (bm) {
        ...
        const progressing = (!!bm._aprbSlowRate && Number(bm._aprbSlowRate) > 0) && !bm._aprbTimeStop && !APRB_isBattleTimeFrozen(bm);
        if (progressing) {
        ...
        // --- Robust fallbacks ---
        // Even if other plugins bypass BattleManager.update/updateTpb, keep frame-based systems consistent.
        // NOTE: APRB_processTbpFrameProgress() already has a once-per-Graphics.frameCount guard.
        //       These fallbacks must NOT run in the same frame, or keepframe/delay timers will double-count.
        try {
          const fcScene = (typeof Graphics !== "undefined" && Graphics.frameCount != null) ? Number(Graphics.frameCount) : -1;
          const tbpRanThisFrame = (fcScene >= 0 && bm._aprbLastTbpProgressFrame === fcScene);
          if (APRB_isBattleTimeFrozen(bm)) {
            // Do not advance delay timers / keepframe while frozen.
          } else if (!tbpRanThisFrame) {
            // --- ActionEndDelay timer (Scene fallback) ---
            ...
            if (STATE_KEEPFRAME_TAG_ENABLE && fcScene >= 0) {
              if (bm._aprbLastKeepFrameSceneFallback !== fcScene) {
                bm._aprbLastKeepFrameSceneFallback = fcScene;
                ...
                for (let i2 = 0; i2 < list2.length; i2++) {
                  ...
                  for (let k2 = 0; k2 < keys2.length; k2++) {
                    ...
                    if (left2 <= 0) {
                      delete m2[sid2];
                      try { b2._aprbSKF_removeSid = sid2; b2._aprbSKF_removeFrame = fcScene; } catch (eMark) {}
                      try { if (b2.isStateAffected && b2.isStateAffected(sid2)) b2.removeState(sid2); } catch (e3) {}
                      // (Removed-state log is handled by removeState hook)
                    } else {
                      m2[sid2] = left2;
                    }
                  }
...
Scene_Battle.prototype._rt_updateRealtimeInput = function() {
    ...
    if (!actor.canMove()) {
      // If action is disabled by state (<APRB_ActionDisable>), block even parry-tap.
      if (actor.isAprbActionDisabled && actor.isAprbActionDisabled()) {
        if (this._rt_zHoldActive) { this._rt_abortZHold(); }
        return;
      }
...
(() => {
  ...
  function aprbTickStateKeepFrame(dtFrames) {
    ...
    if (typeof BattleManager !== "undefined" && BattleManager.APRB_isBattleTimeFrozen && BattleManager.APRB_isBattleTimeFrozen()) return;
...
/* =====================================================================
 * [M.I.P][APRB] battleTimeFreezeFix6
 * 目的: 「メッセージ/イベント/特殊処理中に戦闘時間が進む」を確実に止める。
 * 方針: BattleManager.update は止めない（デッドロック防止）。
 *       APRB のフレーム進行だけを凍結する判定を強化する。
 * ===================================================================== */
(function() {
  "use strict";
  function _aprbSafeBool(fn) { try { return !!fn(); } catch(_) { return false; } }

  // 判定強化版（後勝ちで上書き）
  window.APRB_isBattleTimeFrozen = function(bm) {
    try { if (!bm) bm = (typeof BattleManager !== "undefined") ? BattleManager : null; } catch (_) { bm = null; }

    const explicit = !!(bm && (bm._aprbTimeStop || bm._aprbParryExternalPause || bm._aprbCinematicAfterCommonActive));

    const msgBusy = _aprbSafeBool(() => $gameMessage && $gameMessage.isBusy && $gameMessage.isBusy());
    const troopEv = _aprbSafeBool(() => $gameTroop && $gameTroop.isEventRunning && $gameTroop.isEventRunning());
    const interpRun =
      _aprbSafeBool(() => bm && bm._interpreter && bm._interpreter.isRunning && bm._interpreter.isRunning()) ||
      _aprbSafeBool(() => SceneManager && SceneManager._scene && SceneManager._scene._interpreter && SceneManager._scene._interpreter.isRunning && SceneManager._scene._interpreter.isRunning());
    const commonRes = _aprbSafeBool(() => $gameTemp && $gameTemp.isCommonEventReserved && $gameTemp.isCommonEventReserved());

    // 「特殊処理中」= 戦闘進行が busy な間も含める（時間だけ止める）
    const bmBusy = _aprbSafeBool(() => bm && bm.isBusy && bm.isBusy());
    const lwBusy = _aprbSafeBool(() => bm && bm._logWindow && bm._logWindow.isBusy && bm._logWindow.isBusy());
    const ssBusy = _aprbSafeBool(() => bm && bm._spriteset && bm._spriteset.isBusy && bm._spriteset.isBusy());

    // さらに phase=action 中は、演出を挟んでいる可能性が高いため凍結扱い
    const phaseAction = !!(bm && bm._phase === "action");

    return (explicit || msgBusy || troopEv || interpRun || commonRes || bmBusy || lwBusy || ssBusy || phaseAction);
  };

  try {
    if (typeof BattleManager !== "undefined") {
      BattleManager.APRB_isBattleTimeFrozen = function() { return window.APRB_isBattleTimeFrozen(this); };
    }
  } catch (_) {}
})();
...
//=============================================================================
// [M.I.P][APRB] battleTimeFreezeFix6 - NRP_BattleParallelCommon 互換
// 目的: 戦闘中並列コモン（NRP_BattleParallelCommon等）による「時間進行」を凍結条件に同期させる
// 仕様: BattleManager.APRB_isBattleTimeFrozen() が true の間、Game_Map.updateBattleCommonEvents() を実行しない
//=============================================================================
(() => {
  "use strict";

  function wrapUpdateBattleCommonEventsOnce() {
    if (!Game_Map || !Game_Map.prototype) return;
    const proto = Game_Map.prototype;
    const fn = proto.updateBattleCommonEvents;
    if (typeof fn !== "function") return;
    if (fn._aprbWrappedFreeze) return;

    const wrapped = function() {
      // 戦闘中のみ対象
      if ($gameParty && $gameParty.inBattle && $gameParty.inBattle()) {
        if (BattleManager && BattleManager.APRB_isBattleTimeFrozen && BattleManager.APRB_isBattleTimeFrozen()) {
          return;
        }
      }
      return fn.apply(this, arguments);
    };
    wrapped._aprbWrappedFreeze = true;
    proto.updateBattleCommonEvents = wrapped;
  }

  // BattleManager.update はほぼ必ず毎フレーム通るため、ここで遅延ラップを行う
  const _BM_update = BattleManager.update;
  BattleManager.update = function() {
    // 1回だけラップを試行（NRPが後読みでも戦闘開始後に拾える）
    if (!this._aprbWrappedBattleParallel) {
      wrapUpdateBattleCommonEventsOnce();
      // ラップ済み判定
      const fn = Game_Map && Game_Map.prototype ? Game_Map.prototype.updateBattleCommonEvents : null;
      if (fn && fn._aprbWrappedFreeze) this._aprbWrappedBattleParallel = true;
    }
    return _BM_update.apply(this, arguments);
  };
})();
...
/* =====================================================================
 * [M.I.P][APRB] battleTimeFreezeFix7
 * FIX: battleTimeFreezeFix6 の凍結条件に「bmBusy/phase=action/logBusy/spritesetBusy/interpreterRun」を含めると
 *      StateKeepFrame 等の通常戦闘進行まで凍結してしまうケースがあるため撤去。
 *      仕様要件「イベント/メッセージ等の特殊処理中のみ時間を止める」を満たす最小条件へ戻す。
 *      ※BattleManager.update 自体は止めない（デッドロック防止）。
 * ===================================================================== */
(function(){
  'use strict';
  function _b(v){ try{ return !!v; }catch(_){ return false; } }
  function _safe(fn){ try{ return !!fn(); }catch(_){ return false; } }
  ...
  window.APRB_isBattleTimeFrozen = function(bm){
    ...
    // “イベント/メッセージ” 系：ここだけで凍結する
    const msgBusy  = _safe(() => $gameMessage && $gameMessage.isBusy && $gameMessage.isBusy());
    const troopEv  = _safe(() => $gameTroop && $gameTroop.isEventRunning && $gameTroop.isEventRunning());
    const commonRes= _safe(() => $gameTemp && $gameTemp.isCommonEventReserved && $gameTemp.isCommonEventReserved());

    return (explicit || msgBusy || troopEv || commonRes);
  };
